...
var QA = (function () {
  ...
  const DAILY_UPDATE_SYSTEM = [
    ...
    'Write ONE short message — a sentence or two of lead-in, then the client/QTM names being worked on today (from Doing, plus anything in Action Items or due today/overdue worth flagging).',
    'Group that list into sections, in this order, and skip any section with nothing in it: "Overdue:" (DUE says overdue), "Due today:" (DUE says due today), "This week:" (a DUE later this week, or no DUE but the card is in Doing or Action Items), "Also on the radar:" (everything else worth including). A section label is its own plain line, not a markdown header, and is skipped entirely if there is only one section — a one-item or single-section update should not carry a label with nothing to contrast it against.',
    'For each name, if the card has a DUE value, show it in parentheses right after the name, copied exactly as given (do not reword or re-case it) — then a short clause (roughly 5-12 words) describing what the task actually is, written from its NOTE. Full shape: "Dane Nakama (Due tomorrow) — waiting on K-1s before finishing the projection." Read the NOTE and say what it means in your own words; do not just repeat it verbatim, and do not pad with filler if it is already short. Skip the parentheses entirely for a card with no DUE — never invent one. If a card has no NOTE, list just the name (and its DUE, if any) with no invented description.',
    ...
    'No markdown headers. One item per line, dash-prefixed, except the plain-text section labels above. Plain text, the way a person would actually type a quick message.',
    'You may also be given an image — usually a screenshot of his weekly meeting schedule showing which QTM clients he is meeting with on which days. When a card\'s client/QTM name matches someone the image shows him meeting TODAY, replace that card\'s usual NOTE-based clause with one that says he is updating that QTM for today\'s meeting (e.g. "updating today\'s QTM notes ahead of the call"). When a card\'s client matches someone the image shows meeting on a different day this week, phrase its clause as working on or prepping for that QTM instead (e.g. "prepping for Thursday\'s QTM"). A card whose client cannot be matched in the image keeps its normal NOTE-based description, unchanged — never guess a match. Read the image carefully for client/QTM names and dates; if any of it is unclear, cut off, or not actually meeting information, ignore it rather than guessing. The image only changes each card\'s clause and the lead-in — it never overrides what the board data says is due or overdue, and never moves a card into a different section above.'
  ].join('\n');
